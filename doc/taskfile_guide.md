...
| `task run:release-hud` | Lance en mode release avec le HUD Gallium (FPS/CPU Intel/AMD) activé en surcouche. |
| `task run:prime-hud` | Lance sur carte graphique dédiée NVidia (PRIME) avec MangoHud activé. |
...
| Commande | Description |
|:---|:---|
| `task test:all` | Lancement de tous les tests unitaires et de spécifications du projet. |
...
| `task test:coverage` | Calcule la couverture de code locale via `cargo-llvm-cov` et ouvre le rapport HTML. |
...
| `task lint:all` | Exécute la vérification complète de formatage (`rustfmt`), d'analyse statique (`clippy`) et de la syntaxe de la documentation (`vale`). |
...
| `task profile:valgrind-callgrind` | Lance le simulateur sous Valgrind Callgrind pour profiler l'usage des caches et du CPU. |
| `task profile:heaptrack:gui` | Lance le simulateur sous Heaptrack avec interface graphique pour inspecter les allocations de tas dynamiques. |
| `task profile:heaptrack:cli` | Analyse automatisée en CLI des allocations de tas dynamiques via Heaptrack. |
...
| Commande | Description |
|:---|:---|
| `task bench:all` | Lance la totalité des benchmarks Criterion (moteur DSP et Binauralisation). |
...
| Commande | Description |
|:---|:---|
| `task ai:update-skills` | Régénère et configure les compétences IA locales (Repomix + overlays doc/ai_skills/) pour Antigravity / agy. |
